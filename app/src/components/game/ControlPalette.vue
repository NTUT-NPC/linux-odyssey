...
<template>
  <section class="mt-2 flex flex-row gap-3 justify-center">
    ...
    <button id="reset" class="bg-bg-secondary rounded-lg p-2" @click="reset">
      <font-awesome-icon :icon="['fas', 'arrow-rotate-left']" class="text-text-primary px-1" />
      <p class="text-text-primary inline ml-1 md:flex">重來</p>
    </button>
    <button id="continue" class="bg-bg-secondary rounded-lg p-2" @click="continuePlay">
      <font-awesome-icon :icon="['far', 'circle-right']" class="text-text-primary px-1" />
